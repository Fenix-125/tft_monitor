...
bool is_valid_data(const uint8_t data[PROTOCOL_CHECKED_MASSAGE]) {
    ...
    for (size_t i = PROTOCOL_MASSAGE_SIZE; i < PROTOCOL_CHECKED_MASSAGE; ++i) {
        check_sum |= data[i] << (uint8_t) (PROTOCOL_CHECKED_MASSAGE - i - 1);
    }
    uint16_t sum = 0;
    for (size_t i = 0; i < PROTOCOL_CHECKED_MASSAGE; ++i) {
        sum += data[i];
    }
...
float extract_float2_protocol(const uint8_t data[PROTOCOL_MASSAGE_SIZE]) {
    ...
    for (size_t i = 2; i > 0; --i) {
        res += (float) (data[PROTOCOL_MASSAGE_SIZE - i] << (i - 1));
    }
